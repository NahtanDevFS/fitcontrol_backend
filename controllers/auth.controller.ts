...
export const confirmarUsuarioYActivar = async (req: Request, res: Response) => {
  const { access_token, refresh_token } = req.body;

  if (!access_token || !refresh_token) {
    return res.status(400).json({ error: "Faltan los tokens de sesión." });
  }

  try {
    //Establece la sesión en el backend para verificar que el token es válido
    const {
      data: { session },
      error: sessionError,
    ...
    //Actualiza el estado del usuario en tabla 'usuario'
    ...
    //Cierra la sesión del backend por seguridad
    await supabase.auth.signOut();

    return res
      .status(200)
      .json({ message: "Usuario verificado y activado correctamente." });
  } catch (error: any) {
    console.error("Error al confirmar el usuario:", error);
    return res.status(500).json({
      error: "No se pudo confirmar el usuario.",
      details: error.message,
    });
  }
};
